...
const SectionListItem = ({ data = [], onChange, }) => {
    ...
    const screenWidth = react_native_1.Dimensions.get('window').width - react_native_1.Dimensions.get('window').width / 4;
    const numColumns = 3;
    const gap = 10;
    ...
    const renderSection = (sectionItem, { list }) => {
        ...
        {(list === null || list === void 0 ? void 0 : list.length) > 0 ? (<react_native_1.FlatList data={list} numColumns={numColumns} style={{
                ...
                <react_native_1.Text style={{ textAlign: 'center' }}>{item === null || item === void 0 ? void 0 : item.title}</react_native_1.Text>
