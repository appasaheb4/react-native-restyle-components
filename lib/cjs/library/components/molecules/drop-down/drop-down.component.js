...
exports.Dropdown = react_1.default.forwardRef(({ label, displayKey = 'title', hasError, testID, data, zIndex, unSelectedItemColor = '#ffffff', selectedItemColor = '#F0EFEE', iconColor = '#000000', onSelect, }, ref) => {
    ...
    const renderItem = ({ item, index }) => (<react_native_1.TouchableOpacity style={{
        ...
        <components_1.Text variant="bodyText">{item[displayKey]}</components_1.Text>
      </react_native_1.TouchableOpacity>);
    const renderDropdown = () => {
        return (visible && (<components_1.Box style={{
                flex: 1,
                position: 'absolute',
                top: INPUT_HEIGHT - 4,
                shadowColor: '#000',
    ...
    return (<components_1.Box flexDirection="row" borderColor={hasError ? 'red' : 'gray'} borderWidth={0.4} borderRadius={5} marginTop="xs" marginBottom="xs" alignItems="center" justifyContent="center" height={INPUT_HEIGHT} zIndex={zIndex} testID={testID}>
        <react_native_1.TouchableOpacity ref={DropdownButton} onPress={toggleDropdown} style={{
          ...
          <components_1.Text variant="bodyText" marginHorizontal="ss">
            {value || label}
          </components_1.Text>
          {renderDropdown()}
