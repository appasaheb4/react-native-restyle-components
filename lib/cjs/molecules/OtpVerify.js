...
const OtpVerify = ({ onVerify }) => {
    const [otp, setOtp] = react_1.useState([]);
    const [otpIndex, setOtpIndex] = react_1.useState([]);
    const otpTextInput = react_1.useRef([]);
    react_1.useEffect(() => {
    ...
    return (<>
			<react_native_1.View flexDirection="row" justifyContent="space-between" marginVertical="m" width={react_native_1.Dimensions.get('screen').width - 160}>
